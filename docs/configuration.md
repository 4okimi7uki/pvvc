...
| 環境変数         | config.toml キー | 必須 | 説明                                                        |
| ---------------- | ---------------- | :--: | ----------------------------------------------------------- |
| `GEMINI_API_KEY` | `ai.gemini_key`  |  -   | Gemini API キー。`--llm gemini`（デフォルト）時に使用       |
| `CLAUDE_API_KEY` | `ai.claude_key`  |  -   | Claude API キー。`--llm claude` 時に使用                    |

> `--llm` フラグで使用する LLM を切り替えられます（デフォルト: `gemini`）。対応するAPIキーが未設定の場合はエラーになります。
...
[ai]
gemini_key = "AIza..."
claude_key = "sk-ant-..."  # Claude を使う場合
